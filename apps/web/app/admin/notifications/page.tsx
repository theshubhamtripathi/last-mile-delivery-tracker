...
export default function NotificationsPage() {
  ...
  return (
    <AppShell requireRole="ADMIN">
      ...
      <p className="mt-1 text-sm text-muted">Transactional outbox drained by the worker every 10s. With no API keys, the console provider marks messages sent.</p>
                ...
                {data.map((r) => {
                  ...
                  return (
                    <tr key={n.id} className="border-b border-line last:border-0">
                      <td className="px-4 py-2 text-xs text-muted">{formatDateTime(n.createdAt)}</td>
                      ...
                      <td className="px-4 py-2 font-mono text-[10px] text-faint">{n.providerMessageId ?? '—'}</td>
                    </tr>
                  );
                })}
              </tbody>
