...
export default function AuditPage() {
  ...
  return (
    <AppShell requireRole="ADMIN">
      ...
      <p className="mt-1 text-sm text-muted">Every config change and status override, with before/after state and the actor.</p>
                ...
                {data.map((r) => {
                  ...
                  return (
                    <tr key={a.id} className="border-b border-line last:border-0">
                      <td className="px-4 py-2 text-xs text-muted">{formatDateTime(a.createdAt)}</td>
                      <td className="px-4 py-2 text-xs">{a.actor.fullName} <span className="font-mono text-faint">({a.actor.role})</span></td>
