      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-2">
        <Card>
          ...
          <ul className="mt-3 space-y-1">
            {zones.data?.map((z) => (
              <li key={z.id} className="flex items-center justify-between rounded border border-rule px-3 py-2 text-sm">
                <span className="font-mono">{z.code} · {z.name}</span>
                <span className="font-mono text-xs text-ink/50">{z._count?.serviceAreas ?? 0} areas</span>
              </li>
            ))}
          </ul>
        </Card>
        ...
        <Card>
          ...
          <div className="mt-3 max-h-[520px] overflow-y-auto">
            <table className="w-full text-sm">
              <thead className="sticky top-0 border-b border-rule bg-white text-left font-mono text-xs uppercase tracking-wide text-ink/50">
              ...
              <tbody>
                {areas.data?.map((a) => {
                  ...
                  return (
                    <tr key={area.id} className="border-b border-rule last:border-0">
